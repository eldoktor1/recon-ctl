...
- needs-human: real but requires authentication / 2 OWNED accounts (IDOR/BAC) / an active PoC — give a
  precise operator_plan (e.g. the exact 2-account swap with owned ids only).
      ...
      case "$verdict" in
        ...
        needs-human|needs-account)
          ...
          { [[ -s "$brief" ]] || printf '# Hunter worklist — %s\n\n' "$stamp" > "$brief"
            printf -- '- **[%s] %s** `%s` — %s\n  - %s\n  - OPERATOR: %s\n' "$sev" "$vc" "$url" "$host" "$ev" "${plan:-2-owned-account test; owned ids only; confirm-then-stop}" >> "$brief"; } ;;
      esac
