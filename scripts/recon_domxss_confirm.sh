...
in_scope_now() {
  ...
  if [[ -f "$_sc" ]]; then
    ...
    [[ "$(es "$ES_URL/$INDEX_NAME/_source/$1" 2>/dev/null | jq -r '((.triage_in_scope//false)==true) and ((.triage_pays//false)==true) and ((.triage_out_of_scope//false)!=true) and ((.triage_scan_deny//false)!=true)' 2>/dev/null)" == "true" ]]
  fi
}
