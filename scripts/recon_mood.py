...
def alive_query(qfilter, qshould=None, pool=500):
    body={"size":pool,
      "_source":["host","root_domain","triage_program","triage_payout_tier","triage_score",
                 "triage_true_fresh","tech","triage_classes","triage_signals","host_notes_count"],
...
def write_alive_report(mood, kind, hint, rows, total, stamp, top):
    ...
    for r in rows:
        fr="⚡" if r["fresh"] else "  "
        nt=" 📝" if r["noted"] else ""
        ctx=fmt_list(r["tech"]) or fmt_list(r["classes"]) or fmt_list(r["signals"])
